...
Respond in JSON format:
{
  ...
  async captureViolationScreenshot(employeeId, applicationData, violationSession) {
    try {
      // Take screenshot with headless environment handling
      ...
      try {
        ...
        if (isHeadless) {
          console.log('📸 Headless environment detected for violation screenshot, creating placeholder');
          screenshotBuffer = await this.createViolationPlaceholderScreenshot(applicationData);
          isPlaceholder = true;
        } else {
          screenshotBuffer = await screenshot({ format: 'png' });
        }
      } catch (screenshotError) {
        console.log('📸 Violation screenshot failed, creating placeholder:', screenshotError.message);
        screenshotBuffer = await this.createViolationPlaceholderScreenshot(applicationData);
        isPlaceholder = true;
      }
  ...
  /**
   * Create placeholder screenshot for violation in headless environments
   */
  async createViolationPlaceholderScreenshot(applicationData) {
    const sharp = require('sharp');

    const width = 1920;
    const height = 1080;

    const violationInfo = applicationData || { title: 'Unknown Activity', url: 'Unknown URL' };
    ...
    const svg = `
      <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Header -->
        <rect x="0" y="0" width="100%" height="100" fill="#e53e3e"/>
        <text x="${width/2}" y="40" text-anchor="middle" fill="#ffffff" font-family="Arial" font-size="28" font-weight="bold">
          ⚠️ POLICY VIOLATION DETECTED
        </text>
        <text x="${width/2}" y="70" text-anchor="middle" fill="#fed7d7" font-family="Arial" font-size="16">
          Intelligent Monitoring System Alert
        </text>

        <!-- Violation Details -->
        <rect x="50" y="150" width="${width-100}" height="200" fill="#2d3748" rx="12"/>
        <text x="80" y="190" fill="#e2e8f0" font-family="Arial" font-size="18" font-weight="bold">
          Violation Details:
        </text>
        <text x="80" y="220" fill="#cbd5e0" font-family="Arial" font-size="14">
          Activity: ${violationInfo.title || 'Unauthorized Activity'}
        </text>
        <text x="80" y="245" fill="#cbd5e0" font-family="Arial" font-size="14">
          URL: ${violationInfo.url || 'N/A'}
        </text>
        <text x="80" y="270" fill="#cbd5e0" font-family="Arial" font-size="14">
          Timestamp: ${new Date().toISOString()}
        </text>
        <text x="80" y="295" fill="#cbd5e0" font-family="Arial" font-size="14">
          Environment: Headless Cloud Deployment
        </text>
        <text x="80" y="320" fill="#cbd5e0" font-family="Arial" font-size="14">
          Screenshot: Simulated for compliance
        </text>

        <!-- Simulated Browser Window -->
        <rect x="100" y="400" width="${width-200}" height="${height-500}" fill="#ffffff" rx="8"/>
        <rect x="100" y="400" width="${width-200}" height="40" fill="#f56565" rx="8 8 0 0"/>
        <circle cx="130" cy="420" r="6" fill="#fc8181"/>
        <circle cx="150" cy="420" r="6" fill="#f6e05e"/>
        <circle cx="170" cy="420" r="6" fill="#68d391"/>

        <text x="200" y="425" fill="#ffffff" font-family="Arial" font-size="12" font-weight="bold">
          ${violationInfo.title || 'Violation Activity'}
        </text>

        <!-- Warning message in browser -->
        <text x="${width/2}" y="500" text-anchor="middle" fill="#e53e3e" font-family="Arial" font-size="24" font-weight="bold">
          UNAUTHORIZED WEBSITE ACCESS
        </text>
        <text x="${width/2}" y="530" text-anchor="middle" fill="#718096" font-family="Arial" font-size="16">
          This activity violates company policy
        </text>
      </svg>
    `;
